...
namespace AudioVisualizer.ViewModels
{
    public class MainViewModel : INotifyPropertyChanged
    {
        ...
        private TextStyle _selectedTextStyle = TextStyle.Digital; // Changed default to Digital
        ...
        public VisualizerMode SelectedVisualizerMode
        {
            ...
            set
            {
                if (_selectedVisualizerMode != value)
                {
                    _selectedVisualizerMode = value;
                    OnPropertyChanged();
                    OnPropertyChanged(nameof(IsBarsMode));
                    OnPropertyChanged(nameof(IsOscilloscopeMode));
                    OnPropertyChanged(nameof(IsEddieMode));
                }
            }
        }

        public bool IsBarsMode 
        { 
            get => _selectedVisualizerMode == VisualizerMode.Bars; 
            set { if (value) SelectedVisualizerMode = VisualizerMode.Bars; } 
        }

        public bool IsOscilloscopeMode 
        { 
            get => _selectedVisualizerMode == VisualizerMode.Oscilloscope; 
            set { if (value) SelectedVisualizerMode = VisualizerMode.Oscilloscope; } 
        }

        public bool IsEddieMode 
        { 
            get => _selectedVisualizerMode == VisualizerMode.Eddie; 
            set { if (value) SelectedVisualizerMode = VisualizerMode.Eddie; } 
        }
        ...
        private System.Windows.Media.Brush _barBrush = null!;
        ...
        public class BarData : INotifyPropertyChanged
        {
            ...
            // For Circular/Mirrored logic
            public int Index { get; set; }
            public double Angle { get; set; } // 0-360
        ...
        // Oscilloscope Data
        private System.Windows.Media.PointCollection _waveformPoints = new System.Windows.Media.PointCollection();
        public System.Windows.Media.PointCollection WaveformPoints
        {
            get => _waveformPoints;
        ...
        // Eddie Visualizer Data
        private double _eddieEyeIntensity = 0.0;
        public double EddieEyeIntensity
        {
            get => _eddieEyeIntensity;
            set 
            { 
                _eddieEyeIntensity = value; 
                OnPropertyChanged();
            }
        }

        // Eye Alignment (Adjustable defaults)
        // 0.0 is Center
        private double _eddieEyeHeight = -20.0; 
        public double EddieEyeHeight
        {
            get => _eddieEyeHeight;
            set { _eddieEyeHeight = value; OnPropertyChanged(); }
        }

        private double _eddieEyeGap = 28.0; // Reduced from 40.0 to move closer to center
        public double EddieEyeGap
        {
            get => _eddieEyeGap;
            set { _eddieEyeGap = value; OnPropertyChanged(); }
        }

        // Background Glow (Treble)
        private double _eddieBgGlowIntensity = 0.0;
        public double EddieBgGlowIntensity
        {
            get => _eddieBgGlowIntensity;
            set { _eddieBgGlowIntensity = value; OnPropertyChanged(); }
        }

        private double _eddieBgGlowRadius = 40.0;
        public double EddieBgGlowRadius
        {
            get => _eddieBgGlowRadius;
        ...
        public MainViewModel()
        {
            ...
            {
                SpectrumData.Add(new BarData { Index = i, Angle = (360.0 / BarCount) * i });
            }
            ...
            // Set Initial Styles
            ApplyStyle(_selectedTextStyle);
            // ApplyColor(TextColorOption.White); // Removed to allow ApplyStyle to set default (Green for Digital)
            UpdateBarBrush();
        ...
        private void OnAudioDataAvailable(object? sender, NAudio.Wave.WaveInEventArgs e)
        {
            ...
            // -----------------------------
            // OSCILLOSCOPE LOGIC (Stabilized)
            // -----------------------------
            if (IsOscilloscopeMode)
            {
                // Run on UI Thread to update PointCollection
                System.Windows.Application.Current.Dispatcher.Invoke(() =>
                {
                    var points = new System.Windows.Media.PointCollection();
                    
                    // Trigger Logic: Find the first Zero-Passing (Rising Edge)
                    int triggerIndex = 0;
                    int searchLimit = Math.Min(e.BytesRecorded / 2, 2000); // Only search first half
                    
                    for (int i = 0; i < searchLimit; i+=4) // Jump by 4 (float step)
                    {
                        float val = BitConverter.ToSingle(e.Buffer, i);
                        
                        // Check if we just crossed zero going up
                        if (val <= 0 && i + 4 < e.BytesRecorded)
                        {
                            float nextVal = BitConverter.ToSingle(e.Buffer, i + 4);
                            if (nextVal > 0)
                            {
                                triggerIndex = i;
                                break;
                    ...
                    // Now render N points starting from triggerIndex
                    int pointsToRender = 400; // Match width
                    int step = 1; // 1:1 if possible, or skip if buffer is huge
                    
                    // Dynamic skip if buffer is too long, but usually we want to see the wave detail
                    // Wave at 44.1kHz: 1 cycle of 100Hz is 441 samples.
                    // If we show 400 pixels and step 1, we show ~1 cycle of 100Hz.
                    // For Bass, this is good. For Treble, we might see too much aliasing.
                    
                    // Let's cap step to ensure we fit in buffer
                    if ((e.BytesRecorded - triggerIndex) / pointsToRender > 1) 
                    {
                         // step = (e.BytesRecorded - triggerIndex) / pointsToRender; 
                         // No, let's keep it zoomed in (Time Domain)
                         step = 2; 
                    }

                    for (int x = 0; x < pointsToRender; x++)
                    {
                        int index = triggerIndex + (x * step * 4); // *4 because 4 bytes per float
                        if (index + 4 >= e.BytesRecorded) break;
                        
                        float val = BitConverter.ToSingle(e.Buffer, index);
                        
                        // Visual Scaling: 
                        // Window is 120 high. Center is 60.
                        // Audio is -1.0 to 1.0.
                        // Scale factors: Regular ~0.5. Beat ~0.9.
                        // Let's scale up to fill space.
                        
                        // Reverted Scale to 100
                        double scale = 100.0; 
                        
                        double y = 60 - (val * scale); 
                        
                        // Clamp to prevent drawing outside
        ...
        private void OnRenderTick(object? sender, EventArgs e)
        {
            ...
            for (int i = 0; i < BarCount; i++)
            {
                ...
                if (_currentBars[i] < 0.01) _currentBars[i] = 0.01; 
            ...
            // Treble Logic (Top 20 bars = high frequencies)
            double trebleSum = 0;
            int trebleCount = 20;
            for(int i = BarCount - trebleCount; i < BarCount; i++) trebleSum += _currentBars[i];
            double trebleAmplitude = trebleSum / trebleCount;

            // Update Line Thickness (Oscilloscope)
            if (IsOscilloscopeMode)
            {
                 LineThickness = 2.0 + (BassAmplitude * 6.0);
            }

            // Update Eddie Mode
            if (IsEddieMode)
            {
                // Eyes glow with bass (low frequencies)
                double targetIntensity = (BassAmplitude * 4.0); 
                if (targetIntensity < 0.1) targetIntensity = 0.1; 
                if (targetIntensity > 1.0) targetIntensity = 1.0;
                EddieEyeIntensity = targetIntensity;

                // Background glow with treble (high frequencies)
                double bgTarget = trebleAmplitude * 5.0;
                if (bgTarget > 1.0) bgTarget = 1.0;
                // Smooth transition
                EddieBgGlowIntensity = EddieBgGlowIntensity + (bgTarget - EddieBgGlowIntensity) * 0.3;
                // Pulse the blur radius with treble too (40 to 120)
                EddieBgGlowRadius = 40.0 + (EddieBgGlowIntensity * 80.0);
            }
        }
